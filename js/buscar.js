...
async function mostrartabla(){
    ...
    const querySnapshot = await getDocs(collection(db, "DBdatos2"));
  ...
  <tbody id="cuerpotabla">
    ...
    querySnapshot.forEach((doc) => {
      ...
      <th scope="col">${registros.email}</th>
...
formregistro.addEventListener('submit',async (e)=>{
    ...
    const docRef = doc(db, "DBdatos2", correo);
    ...
    if (docSnap.exists()) {
      ...
      <div class="card-body">
        <h5 class="card-title"> ${docSnap.data().email}</h5>
